...
- Uses Kokoro-82M for normal voice synthesis (fast, good quality)
- Uses LFM 2.5 Audio for premium/high-quality synthesis (slower, better quality)
...
VOICE_PROFILE=normal        # normal (Kokoro-82M) or premium (LFM 2.5)
...
# Force ElevenLabs cloud
briefly brief --voice --engine cloud

# Use premium voice profile
briefly brief --voice --profile premium

# Benchmark both engines
briefly benchmark --runs 5
```
...
```bash
make models         # Download/verify MLX models
make benchmark      # Benchmark voice engines (local vs cloud)
make run-args ARGS="brief --voice --engine local"   # Test local MLX
make run-args ARGS="brief --voice --engine cloud"   # Test ElevenLabs
```
...
# Test voice engines
swift run briefly benchmark --json
swift run briefly brief --voice --engine local
...
The system uses a modular `VoiceEngine` protocol with pluggable implementations:
...
// Available engines
MLXVoiceEngine()        // Local MLX models (default)
ElevenLabsVoiceEngine() // Cloud ElevenLabs API
```
...
- **Architecture**: Peekaboo standard (Executable -> CLI -> Core)
- **Concurrency**: Extensive use of `async/await` and `actor` for thread safety
- **Voice**: Local-first via MLX (defaults to `local` engine)
- **Dependencies**: SwiftPM with local path for GabGab integration
- **Configuration**: Environment variables with `.env` file
